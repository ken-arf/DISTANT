...
def load_dataset(parameters):
    ...
    n = int(len(val_text_data) * 0.8)


    train_text_data.extend(val_text_data[:n])
    train_label_data.extend(val_label_data[:n])

    val_text_data = val_text_data[n:]
    val_label_data = val_label_data[n:]

    
    data = {'train': {'text': train_text_data, 'label': train_label_data},
            'val': {'text': val_text_data, 'label': val_label_data}}
