...
def setup_final_dataset( parameters):
    ...
    document_root = parameters["document_root_dir"]
...
def finetune_classifier(parameters):
    ...
    df_val_first = df_val.iloc[:n]
    df_val_new = df_val.iloc[n:]


    df_train_new = pd.concat([df_train, df_val_first])
    
    df_train_new['split'] = 'train'
    df_val_new['split'] = 'val'
