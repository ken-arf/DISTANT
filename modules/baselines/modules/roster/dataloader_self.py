...
class Dataloader:
    ...
    def load_data(self):
        ...
        aux_texts  = self.dataset['aux_text']
        ...
        aux_texts = [aux_texts[i] for i in index]
        ...
        train_aux_texts = aux_texts[:train_size]
        valid_aux_texts = aux_texts[train_size:]
        ...
        tokenized_datasets = self.datasets.map(
                ...
                batched = True,
                remove_columns = ["text", "aux_text", "bio_label"],
                )
    ...
    def data_collator(self, features):
        ...
        aux_input_ids = [feature["aux_input_ids"] for feature in features]
        # collate aux_input_ids
        pad_token_id = 0
        padding_side = self.tokenizer.padding_side
        if padding_side == "right":
            batch["aux_input_ids"] = [
                list(aux_input_id) + [pad_token_id] * (sequence_length - len(aux_input_id)) for aux_input_id in aux_input_ids
            ]
        else:
            batch["aux_input_ids"] = [
                [pad_token_id] * (sequence_length - len(aux_input_id)) + list(aux_input_id) for aux_input_id in aux_input_ids
            ]
    ...
    def tokenized_and_align_labels(self, examples):
        ...
        tokenized_aux_inputs = self.tokenizer(
                    examples["aux_text"],
                    truncation = True,
                    max_length = 512,
                    is_split_into_words = True,
        ...
        tokenized_inputs["aux_input_ids"] = tokenized_aux_inputs["input_ids"]
