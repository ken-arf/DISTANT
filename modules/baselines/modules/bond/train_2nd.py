...
def train(parameters, name_suffix):
    ...
    # student model
    model_stu = BondNER2nd(parameters, logger)
    # teacher model
    model_tea = BondNER2nd(parameters, logger)

    if parameters['restore_model'] == True:
        model_stu.load_state_dict(torch.load(parameters['restore_model_path'], map_location=torch.device(device)))
        model_tea.load_state_dict(torch.load(parameters['restore_model_path'], map_location=torch.device(device)))

    optimizer = AdamW(model_stu.parameters(), lr=float(parameters['train_lr']))
    ...
    for epoch in range(num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                predictions, probs = model_tea.decode(**batch)

            batch.update({"softlabels":probs})
            probs, loss = model_stu(**batch)
        ...
        for batch_index, batch in tqdm(enumerate(valid_dataloader)):
            with torch.no_grad():
                predictions, probs = model_stu.decode(**batch)
