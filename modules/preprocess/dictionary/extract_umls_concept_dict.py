...
def generate_umls_dict(cui, dict_path):
    ...
    sorted_names = sorted(
        list(set([name for names in all_names for name in names])))
    # delete duplicate atoms
    all_names_clean = [sorted(list(set(names))) for names in all_names]
    # flatten atoms names
    all_names_unfiltered = list(
        set([name for names in all_names_clean for name in names]))
    ...
    for names in all_names_clean2:
        ...
        new_names = [name for name in names if not (
            re.match(prog, name) and (not name in all_names_unfiltered))]
        all_names_clean3.append(new_names)

    # save the result
    umls_atoms = {}
    flatten_atom_names = sorted(
        list(set([name for names in all_names_clean3 for name in names])))
