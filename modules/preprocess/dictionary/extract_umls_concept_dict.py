...
def prepare_umls_dict(parameters):
    ...
    mrsty_rrf = os.path.join(umls_meta_dir, "MRSTY.RRF")
...
def generate_dict(root_cui, dict_path, cui_dict, cui_rel_dict, cui_sty_dict, semantic, max_hop = 1):
    ...
    for hop in range(max_hop):
        ...
        for target_cui in next_cuis:
            cuis = generate_umls_dict_(target_cui, cui_dict, cui_rel_dict, cui_sty_dict, semantic, entries)
...
def main():
    ...
    for entity in parameters['entities'].keys():
        ...
        semantic = parameters['entities'][entity]['Semantic']
        UMLS_cui[entity] = {'RI':ri, 'UMLS_hops': hops, 'Semantic': semantic}

    print(UMLS_cui)

    cui_dict, cui_rel_dict, cui_sty_dict = prepare_umls_dict(parameters)
    ...
    for concept_name, mydict in UMLS_cui.items():
        ...
        generate_dict(cui, dict_path, cui_dict, cui_rel_dict, cui_sty_dict, semantic, max_hop = max_hops)
