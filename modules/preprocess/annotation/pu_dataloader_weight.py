...
class PU_Dataloader:
    ...
    def load_train_only(self):
        # shuffle dataset

        df_train = self.df_train_pos
        df_train = df_train.sample(
            frac=1, replace=False, random_state=self.myseed)
    ...
    def load_data_preset(self):
        ...
        df_train = self.df_train_pos[self.df_train_pos['split'] == 'train']
        df_val = self.df_train_pos[self.df_train_pos['split'] == 'val']
    ...
    def tokenized_and_align_labels(self, examples):
        ...
        for index, (sample_id, offset) in enumerate(zip(sample_map, offset_mapping)):
            ...
            weight = int(examples["weight"][sample_id])
