...
def apply_group(df):
    ...
    df_entities = df.groupby(['text', 'pmid'], as_index=False).agg(
        {'entities': ','.join})
    df_start_chars = df.groupby(['text', 'pmid'], as_index=False).agg({
        'start_chars': ','.join})
    df_end_chars = df.groupby(['text', 'pmid'], as_index=False).agg(
        {'end_chars': ','.join})
    df_label = df.groupby(['text', 'pmid'], as_index=False).agg(
        {'label': ','.join})
...
def main():
    ...
    train_csv = os.path.join(
        parameters["corpus_dir"], parameters["train_csv_name"])

    df_train = pd.read_csv(train_csv, index_col=0)
    df_train = df_train[["entities", "start_chars",
                         "end_chars", "text", "pmid", "label"]]
    ...
    df_train_clean.to_csv(os.path.join(
        parameters["corpus_dir"], f"{basename}_group_clean.csv"))
    ...
    for index, row in df_train_clean.iterrows():
     ...
     #   print(row)
        ...
        char_offsets = [(int(start), int(end))
