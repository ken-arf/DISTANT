...
def classify_unknown_samples(model_dir, thres_pert = 5, count = 1):
    ...
    for i in range(count):
        candidate[i] = extract_unknown_samples(os.path.join(model_dir, f"train_neg_{i}.csv"), thres_pert)
    ...
    for i in range(1,count):
        df = candidate[i]['df_pos']
        df_merge_neg = pd.merge(df_merge_neg, df, left_index=True, right_index=True)

    cols_prob = [col for col in df_merge_pos.columns if col.startswith('max_prob')]
    cols_index = [col for col in df_merge_pos.columns if col.startswith('max_index')]

    pos_index = df_merge_pos.index.tolist()
    neg_index = df_merge_neg.index.tolist()
    pos_label = df_merge_pos[cols_index[0]].values.tolist()

    result = {'pos_index': pos_index, 'neg_index': neg_index, 'pos_label': pos_label}
...
def extract_neg_index(model_dir, thres_pert = 15, count = 1):
    ...
    df_neg_true = {}
    for i in range(count):
        df_neg_true[i] = extract_true_neg_candidate(os.path.join(model_dir, f"train_neg_{i}.csv"), thres_pert)
